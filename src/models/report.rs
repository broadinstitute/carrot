...
//! The actual report entity only holds metadata, and the actual description of what will be
...
/// report_id and created_at are populated automatically by the DB
...
/// associated with the specified report.  If there is not, metadata can also be modified
...
impl ReportData {
    ...
    pub fn find(
        ...
        if let Some(param) = params.metadata {
            query = query.filter(metadata.eq(param));
        }
        ...
        if let Some(sort) = params.sort {
            ...
            for sort_clause in sort {
                match &*sort_clause.key {
                    ...
                    "metadata" => {
                        if sort_clause.ascending {
                            query = query.then_order_by(metadata.asc());
                        } else {
                            query = query.then_order_by(metadata.desc());
                        }
                    }
                    "created_at" => {
                        if sort_clause.ascending {
    ...
    /// `params`.  Fails if trying to update metadata and there are nonfailed run_reports associated
    /// with this report
    ...
    pub fn update(
        ...
        // If trying to update the metadata, verify that no non-failed run_reports exist for this
        // report
        if matches!(params.metadata, Some(_)) {
            match Self::has_nonfailed_run_reports(conn, id) {
                // If there is a nonfailed run_report, return an error
                Ok(true) => {
                    let err = UpdateError::Prohibited(String::from("Attempted to update metadata when a non-failed run_report exists for this template.  Doing so is prohibited"));
...
mod tests {
    ...
    fn find_with_metadata() {
        ...
        let test_query = ReportQuery {
            ...
            name: None,
            description: None,
            metadata: Some(json!({"metadata":[{"test1":"test"}]})),
            created_before: None,
            created_after: None,
            created_by: None,
            sort: None,

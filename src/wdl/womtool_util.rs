...
mod tests {
    ...
    fn test_womtool_parse() {
        let parsed_wdl = parse(&Path::new("testdata/wdl_to_parse.wdl").to_str().unwrap()).unwrap();
        // Load expected output from file
        let expected_output = read_to_string(Path::new("testdata/parsed_wdl_to_parse.txt")).unwrap();

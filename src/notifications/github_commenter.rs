...
impl GithubCommenter {
    ...
    /// Posts a comment to issue `issue_number` on GitHub repo `repo` with owner `owner`, containing a
    /// message notifying that `report_map` has finished for run(s) of test with `test_name`
    /// Returns an error if creating the message or posting it to GitHub fails
    pub async fn post_report_map_finished_comment(
        &self,
        owner: &str,
        repo: &str,
        issue_number: i32,
            ...
            match &report_map.results {
                Some(report_results) => {
                    ...
                    for (report_key, uri) in report_results.as_object().unwrap_or_else(|| panic!(
                        "Results for report map for {} with id {} and report_id {} not formatted as json object",
                        ...
                        let uri_string = uri.as_str().unwrap_or_else(|| panic!(
                            "Result uri for key {} for report_map for {} with id {} and report_id {} not formatted as string",
                            report_key,
                ...
                None => json!(report_map).to_string(),
            }
        };
        ...
        let comment_body = format!(
            "### 🥕CARROT🥕 report map {} finished\nfor test {} ({}: {})\n{}",
            report_name, test_name, report_map.entity_type, report_map.entity_id, report_data
        );
        Ok(self
            .client
            .post_comment(owner, repo, issue_number, &comment_body)
...
mod tests {
    ...
    #[actix_rt::test]
    async fn test_post_report_map_finished_comment() {
        // Get client
        let client = Client::default();
        // Create a github client
        let github_client = GithubClient::new("user", "aaaaaaaaaaaaaaaaaaaaaa", client);
        ...
        // Create a run to test with
        let test_report_map = ReportMapData {
        ...
        let request_body = json!({
            "body":
                format!(
                    "### 🥕CARROT🥕 report map test_report finished\nfor test Test Name (run: {})\n{}",
                    test_report_map.entity_id, expected_results
                )
        });
    ...
    #[actix_rt::test]
    async fn test_post_report_map_finished_comment_run_group() {
        // Get client
        let client = Client::default();
        // Create a github client
        let github_client = GithubClient::new("user", "aaaaaaaaaaaaaaaaaaaaaa", client);

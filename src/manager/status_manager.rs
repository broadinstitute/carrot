...
pub fn manage(db_pool: DbPool, client: Client, channel_recv: mpsc::Receiver<()>) -> Result<(), StatusManagerError> {
    lazy_static! {
        ...
        match unfinished_runs {
            ...
            Ok(runs) => {
                ...
                for run in runs {
                    ...
                    if let Some(_) = check_for_terminate_message(&channel_recv) { return Ok(())};
                    // Check and update status
                    debug!("Checking status of run with id: {}", run.run_id);
                    if let Err(e) = block_on(check_and_update_status(&run, &client, &db_pool.get().unwrap())) {
            ...
            Err(e) => {
                ...
                if consecutive_failures > *ALLOWED_CONSECUTIVE_STATUS_CHECK_FAILURES {
                    ...
                    return Err(StatusManagerError{msg: String::from("Exceed allowed consecutive failures")});
                }
    ...
    if status != run.status.to_string() {
        ...
        if &*status == "succeeded" {
            let outputs = match metadata.get("outputs") {
                ...
                None => return Err(UpdateStatusError::Cromwell(String::from("Cromwell metadata request did not return outputs")))
            };
...
fn get_end(metadata: &Map<String,Value>) -> Result<NaiveDateTime, UpdateStatusError> {
    let end = match metadata.get("end") {
        ...
        None => return Err(UpdateStatusError::Cromwell(String::from("Cromwell metadata request did not return end")))
    };
    match NaiveDateTime::parse_from_str(end, "%Y-%m-%dT%H:%M:%S%.fZ") {
        Ok(end) => Ok(end),
        Err(_) => return Err(UpdateStatusError::Cromwell(format!("Failed to parse end time from Cromwell metadata: {}", end)))
    }
}

fn fill_results(outputs: &Map<String,Value>, run: &RunData, conn: &PgConnection) -> Result<(), UpdateStatusError> {
    ...
    let template_results = match TemplateResultData::find_for_test(conn, run.test_id) {
        ...
        Err(e) => {
            return Err(UpdateStatusError::DB(format!("Failed to load result mappings from DB with error: {}", e)));
        }
    };
        ...
        match outputs.get(&*template_result.result_key) {
            ...
            Some(output) => {
                let output = match output.as_str() {
                    ...
                    None => {
                        error!("Failed to parse output {} as string for run {}, had value: {}", template_result.result_key, run.run_id, output);
            ...
            None => {
                error!("Run with id {} missing output in Cromwell outputs for key {}", run.run_id, template_result.result_key);
                missing_keys_list.push(template_result.result_key.clone());
            }
        }
    }
    ...
    if let Err(_) = RunResultData::batch_create(conn, result_list) {
        return Err(UpdateStatusError::DB(format!("Failed to write results to DB for run {}", run.run_id)));
    }

    // If there were missing keys, return an error
    if missing_keys_list.len() > 0 {
        return Err(UpdateStatusError::Cromwell(format!("Cromwell returned outputs with the following keys missing or failing to parse: {}", missing_keys_list.join(","))));
    }

    ...
    match RunReportData::find_by_run_and_report(conn, run_id, report_id) {
        Ok(existing_run_report) => {
            ...
            // Otherwise, return an error
                ...
                return Err(Error::Prohibited(format!("A run_report record already exists for run_id {} and report_id {}", run_id, report_id)));
            }
        },
...
fn create_generator_wdl(
    ...
    for position in 0..sections.len() {
        ...
        for (input_name, report_input) in section_inputs {
            ...
            workflow_inputs_lines.push(format!(
                "        {} {}",
                report_input.input_type, wdl_input_name
            ));
            // [~input_sizes~]
            ...
            // [~call_inputs~]
            call_inputs_lines.push(format!(
                "            {} = {}",
                wdl_input_name, wdl_input_name
            ));
        }
...
fn create_input_json(
    ...
    for position in 0..sections.len() {
        ...
        for (input_name, report_input) in section_inputs {
            // Make a version of the input name that is prefixed with position so we can avoid the
            // problem of having multiple inputs with the same name
            let wdl_input_name = format!("{}.{}", GENERATOR_WORKFLOW_NAME, get_section_wdl_input_name(position, input_name));
...
mod input_map {
        ...
        for section_name in input_map.keys() {
            ...
            for (input_name, input_value) in section_input_map {
                ...
                if input_value.starts_with("test_input:") {
                    ...
                    let report_input = get_report_input_from_input(
                        input_value,
                        &run_test_input,
                        &test_wdl_input_types,
                        &template.test_wdl,
                        "test",
                    )?;
                ...
                // result type map
                    ...
                    // Parse and extract the input indicated by input_value
                    let report_input = get_report_input_from_results(
                        input_value,
                        &run_result_map,
                        &results_with_types,
                        &run.results,
    ...
    fn get_result_types_map_for_template(
        conn: &PgConnection,
        template_id: Uuid,
    ) -> Result<HashMap<String, &str>, diesel::result::Error> {
        ...
        for result in ResultData::find_for_template(conn, template_id)? {
            ...
            results.insert(
                result.name,
                convert_result_type_to_wdl_type(&result.result_type),
            );
        }
        Ok(results)
    }
    ...
    fn get_report_input_from_input(
        ...
        let input_type = match input_types.get(input_value_parsed) {
            ...
            None => {
                let err_msg = format!(
                    "{} WDL {} does not contain an input called {}",
                    context, wdl_location, input_value_parsed
                );
                error!("{}", err_msg);
                return Err(Error::Inputs(err_msg));
            }
    ...
    fn get_report_input_from_results(
        input_value: &str,
        run_result_map: &Map<String, Value>,
        results_with_types: &HashMap<String, &str>,
        run_results: &Option<Value>,
    ) -> Result<ReportInput, Error> {
        ...
        let actual_input_value =
            get_value_from_map_as_string(&run_result_map, input_value_parsed, "results")?;
        ...
        let input_type = match results_with_types.get(input_value_parsed) {
            ...
            None => {
                let err_msg = format!(
                    "Results {:?} do not contain an input called {}",
                    run_results, input_value_parsed
                );
                error!("{}", err_msg);
                return Err(Error::Inputs(err_msg));
            }
...
mod tests {
    ...
    use crate::manager::report_builder::{create_generator_wdl, create_input_json, create_report_template, Error, create_run_report};
    ...
    use crate::custom_sql_types::{RunStatusEnum, ResultTypeEnum, ReportStatusEnum};
    ...
    fn insert_test_run_report_failed(conn: &PgConnection, run_id: Uuid, report_id: Uuid) -> RunReportData {
    ...
    fn insert_test_run_report_nonfailed(conn: &PgConnection, run_id: Uuid, report_id: Uuid) -> RunReportData {
    ...
    async fn create_run_report_failure_wdl_download() {
        ...
        assert!(matches!(result_run_report, Error::Womtool(crate::validation::womtool::Error::Request(_))));
    }
    ...
    async fn create_run_report_failure_no_run() {
        ...
        assert!(matches!(result_run_report, Error::DB(diesel::result::Error::NotFound)));
    }

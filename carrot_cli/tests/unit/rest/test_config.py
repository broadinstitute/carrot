...
from carrot_cli.rest import config as config_rest, request_handler
...
def test_get_cromwell_address():
    ...
    mockito.when(request_handler).send_request("GET", address, expected_format=request_handler.ResponseFormat.TEXT).thenReturn(
        "example.com/cromwell"
    )

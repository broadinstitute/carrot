...
def get_id_from_id_or_name_and_handle_error_data(request):
    ...
    mockito.when(pipelines).find(name=request.param["id_or_name"], limit=2).thenReturn(
        request.param["request_return"]
    )
    return request.param


def test_get_id_from_id_or_name_and_handle_error(
    get_id_from_id_or_name_and_handle_error_data, caplog
):
